...
**Overlay:** C-06 2026-09-17 (does not rewrite RUN 1–12)  
...
| INV-08 | PASS classified (C-06 overlay 2026-09-17) | Prior: C-05 P2 + Brand Payouts v1 + Wave B postgres **PASS 3/3**. Overlay: C-06 GET workspace in lineage (P1–P3 unit + P6 postgres **1/1** on disposable `c06_recovery`; FE P4–P5 + shell). Provider-disabled; no Razorpay/ledger. Live provider remains INV-10. |
| INV-09 | PASS classified | Settings contact proven. Leftover collab shipping HTTP+service `410`. C-04 destination snapshot consumption. Parent reconfirm 2026-09-11 16:50: BE INV-09 static **3/3**, `c04-runtime`+fulfillment **15/15**, FE `c04-frontend` **6/6**. Auth security static+unit **14/14**. Do not invent a live fulfillment join. Destination postgres not re-run this pass. |
...
| INV-11 | PASS classified | Accepted IN feature API clients use `authenticatedFetch` → `/api/v1`. Brand Home / Creator Home aggregators only. Brand Home fail-closed for Creator (smoke). C-06 overlay 2026-09-17: `creator-payouts-client.ts` is **IN** (FE architecture test **3/3**; list file still uncommitted at record). OUT Co-Pilot/Centre excluded | |

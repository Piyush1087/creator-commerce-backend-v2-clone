...
| cross-module invariant suite execution | PARTIAL | postgres INV-01/02/03/04/12/06/07 PASS (prior). INV-08/09/10/11 PARTIAL; INV-13 PASS classified (leftover journey writers retired; tables retained) (`11-invariant-results.md`) |
...
| backend lint | PASS classified | 712 prettier — **accepted this freeze** `PREEXISTING_ACCEPTED_DEBT` (do not `--fix`). Later eslint without prettier: same prettier-only farm stays accepted |

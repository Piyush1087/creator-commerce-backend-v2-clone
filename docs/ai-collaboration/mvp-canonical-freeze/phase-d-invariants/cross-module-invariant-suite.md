...
| INV-09 | Settings shipping/contact → Collaboration fulfillment consumption | C-05 + C-04 | `creator-profile-contact-settings.tsx` | C-05 profile/contact APIs. Leftover collab shipping HTTP+service `410`. C-04 fulfillment does not read `CreatorShippingAddress` | FE contact tests; BE `inv-09-shipping-disposition.static.test.ts` | **PARTIAL** — competing write retired; consumption still not the C-05 address row |
...
Suite is **defined**. Execution is PARTIAL overall (INV-08/09/10 PARTIAL). INV-11 is PASS classified for accepted IN clients. INV-13 leftover journey writers are PASS classified (tables retained). C-02A / C-04 / Brand Payouts v1 are in lineage. That still blocks freeze PASS.

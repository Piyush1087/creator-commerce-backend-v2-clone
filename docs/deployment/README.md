...
# Brand escrow / Razorpay (loaded into ECS via sst.config.ts apiEnvironment)
RAZORPAY_API_KEY_ID=rzp_test_...
RAZORPAY_API_KEY_SECRET=...
RAZORPAY_WEBHOOK_SECRET=...
# EXTERNAL_API_TIMEOUT_MS=10000
```

Frontend deploy (`creator-commerce-frontend-v2`): set `VITE_RAZORPAY_KEY_ID` (same public key id as backend) in that repo’s `.env` before `sst deploy` — baked into the static build.

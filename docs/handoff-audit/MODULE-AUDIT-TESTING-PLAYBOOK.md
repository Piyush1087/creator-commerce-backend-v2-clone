...
Copy the file set below into `docs/handoff-audit/<area>/` and fill it — do not invent ad-hoc filenames.
...
| `ui-verification.md` | Local click-through (R1…Rn); operator-only |
...
| `../.logs/` | Tee’d full logs (`recon-*`, `be-*`, `fe-*`) |
...
| `ENVIRONMENT_BLOCKED` | Could not prove (missing disposable DB, secrets, flaky fixture pollution) — **not** a Product conflict |
...
1. **Do not skip database tests.** Set the suite’s env flags / `*_DATABASE_URL`s so postgres suites run (`skip count: 0`). Document env in test-results.  
...
6. **Scoped suites first** (module paths), then typecheck / build / lint. Full `npm test` only when asked — it is slower and noisier with unrelated origin debt.  
...
10. UI smoke: only where locally executable; cite `ui-verification.md` rows. If not re-run, say **prior PASS** with date — do not invent clicks.  
...
```markdown
### N. <Invariant name> — **PASS|FIXED|GENUINE_AUTHORITY_CONFLICT|ENVIRONMENT_BLOCKED**
...
5. UI smoke (or “prior / not re-run”)  

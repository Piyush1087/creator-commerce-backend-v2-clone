-- C-04 M1: converge the accepted shared Collaboration aggregate onto the
-- accepted C-03 history. This migration is additive and preserves legacy rows.
...
CREATE TYPE "CollaborationNegotiationState" AS ENUM ('NOT_REQUIRED', 'AWAITING_CREATOR_PROPOSAL', 'AWAITING_BRAND_DECISION', 'AWAITING_CREATOR_DECISION', 'LOCKED', 'FAILED');
CREATE TYPE "CollaborationPaymentRail" AS ENUM ('PLATFORM_ESCROW');
CREATE TYPE "CollaborationSecurementState" AS ENUM ('NOT_REQUIRED', 'AWAITING_ESCROW_FUNDING', 'PROCESSING_FUNDING', 'AWAITING_PAYOUT_DETAILS', 'COMPLETED', 'BLOCKED');
...
CREATE INDEX "collaboration_commercial_agreements_pricing_tier_snapshot_business_country_code_snapshot_idx" ON "collaboration_commercial_agreements"("pricing_tier_snapshot", "business_country_code_snapshot");
...
CREATE UNIQUE INDEX "collaboration_deliverable_executions_collaboration_id_source_brief_deliverable_id_key" ON "collaboration_deliverable_executions"("collaboration_id", "source_brief_deliverable_id");
...
CREATE INDEX "collaboration_deliverable_executions_source_brief_deliverable_id_idx" ON "collaboration_deliverable_executions"("source_brief_deliverable_id");
CREATE UNIQUE INDEX "collaboration_submission_versions_deliverable_execution_id_version_number_key" ON "collaboration_submission_versions"("deliverable_execution_id", "version_number");
CREATE INDEX "collaboration_submission_versions_review_state_review_deadline_at_idx" ON "collaboration_submission_versions"("review_state", "review_deadline_at");
CREATE INDEX "collaboration_submission_versions_deliverable_execution_id_submitted_at_idx" ON "collaboration_submission_versions"("deliverable_execution_id", "submitted_at");
CREATE UNIQUE INDEX "collaboration_publishing_executions_deliverable_execution_id_key" ON "collaboration_publishing_executions"("deliverable_execution_id");
CREATE INDEX "collaboration_publishing_executions_state_idx" ON "collaboration_publishing_executions"("state");
CREATE UNIQUE INDEX "collaboration_publishing_evidence_publishing_execution_id_sequence_key" ON "collaboration_publishing_evidence"("publishing_execution_id", "sequence");
CREATE INDEX "collaboration_publishing_evidence_publishing_execution_id_submitted_at_idx" ON "collaboration_publishing_evidence"("publishing_execution_id", "submitted_at");
...
ALTER TABLE "collaboration_deliverable_executions" ADD CONSTRAINT "collaboration_deliverable_executions_source_brief_deliverable_id_fkey" FOREIGN KEY ("source_brief_deliverable_id") REFERENCES "campaign_brief_deliverables"("deliverable_id") ON DELETE RESTRICT ON UPDATE CASCADE;
...
ALTER TABLE "collaboration_publishing_executions" ADD CONSTRAINT "collaboration_publishing_executions_deliverable_execution_id_fkey" FOREIGN KEY ("deliverable_execution_id") REFERENCES "collaboration_deliverable_executions"("id") ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE "collaboration_publishing_evidence" ADD CONSTRAINT "collaboration_publishing_evidence_publishing_execution_id_fkey" FOREIGN KEY ("publishing_execution_id") REFERENCES "collaboration_publishing_executions"("id") ON DELETE CASCADE ON UPDATE CASCADE;

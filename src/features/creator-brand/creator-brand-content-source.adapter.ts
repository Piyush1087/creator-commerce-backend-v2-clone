...
const derived = z
  ...
  .strict();
export type CreatorBrandAdmittedSource = {
  ownerScopeId: string;
  subjectId: string;
  subject: {
...
export const CreatorBrandAdmittedSourceSchema = z
  .object({
...
@Injectable()
export class CreatorBrandContentSourceAdapter {
  ...
  async readInTransaction(
  ...
  ): Promise<CreatorBrandAdmittedSource | null> {
    ...
    const current = await readCreatorBrandCurrent(
      tx,
      scope.id,
      "creator_content",
    );
    ...
    const admitted: CreatorBrandAdmittedSource["posts"] = [];
    ...
    return CreatorBrandAdmittedSourceSchema.parse({
      ...result,
      manifestHash: sha256Canonical(result),
    });
  }
  private fail(code: string): never {
    throw new Error(`CREATOR_BRAND_SOURCE_${code}`);
  }
}

...
export type CreatorBrandCurrentRow = {
...
export async function readCreatorBrandCurrent(
  tx: Prisma.TransactionClient,
  ownerScopeId: string,
  objectSemanticId: string,
): Promise<CreatorBrandCurrentRow[]> {
  const rows = await tx.$queryRaw<
    Array<{ row: CreatorBrandCurrentRow; createdAt: Date }>
  >(Prisma.sql`

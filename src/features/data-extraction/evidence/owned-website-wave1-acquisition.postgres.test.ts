...
const databaseUrl =
  process.env.DE_W1_0D_DATABASE_URL ?? process.env.DE_W1_0C_DATABASE_URL;
...
describePostgres("DE-W1.0D durable owned-site acquisition", () => {
  ...
  it("persists Resource, Capture, content, provider lineage and capability scope without Evidence", async () => {
    ...
    const result = await service.request(
      request(brandId, root, `request:${randomUUID()}`),
    );
    ...
    const captures = await prisma.dataExtractionCapture.findMany({
      where: { brandId },
    });
    const artifacts = await prisma.dataExtractionContentArtifact.findMany({
      where: { brandId },
    });
    const providerLinks =
      await prisma.dataExtractionProviderExecutionLink.findMany({
        where: { brandId },
      });
    const scope = await prisma.dataExtractionCapabilityResource.findMany({
      where: { brandId },
    });
    ...
    expect(artifacts.some((row) => row.kind === "ACQUIRED_SOURCE_BODY")).toBe(
      true,
    );
  ...
  it("same request key replays prepared lineage without duplicate captures or provider attempts", async () => {
    ...
    const first = await service.request(
      request(brandId, "https://example.org/", key),
    );
    ...
    expect(
      await prisma.dataExtractionCapture.count({ where: { brandId } }),
    ).toBe(first.captureRefs!.length);
  });
  ...
  it("material request-key mismatch returns IDEMPOTENCY_CONFLICT", async () => {
    ...
    await expect(
      service.request(
        request(brandId, "https://example.net/", key, "FORCE_RECAPTURE"),
      ),
  ...
  it("REUSE_ALLOWED reuses existing captures across a new capability execution", async () => {
    ...
    expect(second.capabilityExecutionRef).not.toBe(
      first.capabilityExecutionRef,
    );
  ...
  it("FORCE_RECAPTURE creates new Capture history for the same Resource", async () => {
    ...
    const resourceCount = await prisma.dataExtractionResource.count({
      where: { brandId },
    });
    const captureCount = await prisma.dataExtractionCapture.count({
      where: { brandId },
    });
    expect(resourceCount).toBe(first.resourceRefs!.length);
    expect(captureCount).toBe(
      first.captureRefs!.length + second.captureRefs!.length,
    );
  });
  ...
  it("failed root acquisition keeps failed attempt lineage and completes execution UNAVAILABLE", async () => {
    ...
    const execution =
      await prisma.dataExtractionCapabilityExecution.findFirstOrThrow({
        where: { brandId },
      });
    const providerLinks =
      await prisma.dataExtractionProviderExecutionLink.count({
        where: { brandId },
      });
  ...
  it("same owned URL is Brand-isolated and never cross-reuses another Brand Resource/Capture", async () => {
    ...
    const a = await service.request(
      request(brandA, root, `request:${randomUUID()}`),
    );
    const b = await service.request(
      request(brandB, root, `request:${randomUUID()}`),
    );

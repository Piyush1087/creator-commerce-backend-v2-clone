...
export interface DataExtractionEvidenceNormalizer {
  ...
  normalize(input: DataExtractionNormalizationInput): DataExtractionNormalizationResult;
}
...
function sentences(text: string): string[] {
  ...
  return [...new Map(lines.map((line) => [canonicalSemanticText(line), line])).values()];
}
...
function sourceRepresentativeness(
  ...
  if (mode === "OFFERING") {
    ...
    if (["PORTFOLIO_OVERVIEW", "CATEGORY_OVERVIEW", "SERVICE_OVERVIEW", "SOLUTIONS_OVERVIEW", "PRICING_PLANS"].includes(role)) {
      return "PERSISTENT_BRAND_LEVEL";
    }
  }
  if (mode === "COMPANY" && ["ABOUT_COMPANY", "BRAND_STORY", "MISSION_VALUES", "COMPANY_OVERVIEW"].includes(role)) {
    return "PERSISTENT_BRAND_LEVEL";
  }
  if (mode === "LANGUAGE" && representativePage(source.resource)) return "PERSISTENT_BRAND_LEVEL";
  if (mode === "MESSAGING" && ["HOMEPAGE", "ABOUT_COMPANY", "BRAND_STORY", "MISSION_VALUES"].includes(role)) {
    return "PERSISTENT_BRAND_LEVEL";
  }
  if (role === "CAMPAIGN_LANDING") return "CONTEXT_SPECIFIC";
  if (role === "OFFERING_DETAIL") return "OFFERING_SPECIFIC";
  return representativePage(source.resource) ? "CONTEXT_SPECIFIC" : "INCIDENTAL";
}
...
function polarityForStatement(text: string): EvidencePolarity | undefined {
  ...
  if (/\b(never|must not|do not|don't|cannot|can't|prohibit|prohibited|avoid)\b/.test(normalized)) {
    return "RESTRICTION";
  }
  if (/\b(not available|not supported|no longer|does not|doesn't)\b/.test(normalized)) {
...
function draft(
  ...
  return {
    ...
    ...(options?.parentEvidenceRefs
      ...
      : {}),
    ...(options?.conflictFamily ? { conflictFamily: options.conflictFamily } : {}),
  };
}

function messagingRole(text: string): string | null {
  const value = canonicalSemanticText(text);
  if (/\b(mission|vision|purpose|we believe|our story)\b/.test(value)) return "BRAND_STORY_OR_MISSION";
  if (/\b(we help|we enable|we make|we create|we build|built for|designed for|made for)\b/.test(value)) return "BRAND_PROPOSITION";
  if (/\b(leading|trusted|speciali[sz]e|focused on|dedicated to|known for)\b/.test(value)) return "BRAND_POSITIONING";
  if (/\b(you|your)\b/.test(value) && /\b(get|discover|find|choose|start|save|grow|improve|enjoy|experience|book|shop)\b/.test(value)) {
    return "CUSTOMER_GUIDANCE";
  }
  if (/\b(certified|award|trusted by|years? of|customers?|clients?|patients?)\b/.test(value)) {
...
export class BrandMessagingNormalizer implements DataExtractionEvidenceNormalizer {
  ...
  normalize(input: DataExtractionNormalizationInput): DataExtractionNormalizationResult {
    ...
    for (const source of input.sources) {
      if (["LEGAL", "TESTIMONIAL", "POLICY"].includes(source.resource.pageRole ?? "")) continue;
      for (const text of sentences(source.normalizedText).slice(0, 80)) {
        ...
        if (drafts.filter((item) => item.source === source).length >= MAX_ITEMS_PER_RESOURCE) break;
      }
    }
    return { drafts: applyRepeatedRepresentativeness(drafts), reasonCodes: drafts.length ? [] : ["NO_QUALIFYING_BRAND_MESSAGING"] };
  }
}

function companyStatementClass(text: string): string | null {
  const value = canonicalSemanticText(text);
  if (/\b(founded|established|since \d{4}|started in|began in|our journey)\b/.test(value)) return "HISTORY_OR_FOUNDING";
  if (/\b(mission|purpose|we believe|we exist to)\b/.test(value)) return "MISSION_OR_PURPOSE";
  if (/\b(headquartered|based in|operate in|operates in|offices? in|locations? in)\b/.test(value)) return "OPERATING_MODEL";
  if (/\b(we serve|serves|serving|our customers|our clients|our patients)\b/.test(value)) return "CAPABILITY_OR_SCOPE_STATEMENT";
  if (/\b(we are|is a company|is an? (?:platform|company|brand|business|provider|service)|speciali[sz]es? in)\b/.test(value)) return "COMPANY_DESCRIPTION";
  if (/\b(category|industry|market|sector)\b/.test(value)) return "CATEGORY_OR_BUSINESS_CONTEXT";
  if (/\b(our values|principles|we value)\b/.test(value)) return "STATED_PRINCIPLE";
  return null;
}
...
export class BrandCompanyContextNormalizer implements DataExtractionEvidenceNormalizer {
  ...
  normalize(input: DataExtractionNormalizationInput): DataExtractionNormalizationResult {
    const drafts: NormalizedEvidenceDraft[] = [];
    for (const source of input.sources) {
      if (["LEGAL", "TESTIMONIAL"].includes(source.resource.pageRole ?? "")) continue;
      for (const text of sentences(source.normalizedText).slice(0, 100)) {
        ...
        const assertionNature = statementClass === "STATED_PRINCIPLE" || statementClass === "MISSION_OR_PURPOSE"
        ...
        const subjectScope = /\b(headquartered|based in|operate in|locations? in)\b/i.test(text)
    ...
    return { drafts: applyRepeatedRepresentativeness(drafts), reasonCodes: drafts.length ? [] : ["NO_QUALIFYING_COMPANY_CONTEXT"] };
  }
}

function offeringCandidate(text: string, source: DataExtractionNormalizationSource): boolean {
  const role = source.resource.pageRole ?? "OTHER";
  if (["PORTFOLIO_OVERVIEW", "CATEGORY_OVERVIEW", "SERVICE_OVERVIEW", "SOLUTIONS_OVERVIEW", "PRICING_PLANS", "OFFERING_DETAIL"].includes(role)) {
    return text.length <= 220;
  }
  return /\b(product|service|solution|plan|pricing|subscription|collection|package|membership|book|shop)\b/i.test(text);
}
  ...
  normalize(input: DataExtractionNormalizationInput): DataExtractionNormalizationResult {
    ...
    for (const source of input.sources) {
      ...
      for (const text of sentences(source.normalizedText).slice(0, 80)) {
        ...
        const generalizationScope = role === "OFFERING_DETAIL"
          ? "SINGLE_OFFERING"
          : ["PORTFOLIO_OVERVIEW", "CATEGORY_OVERVIEW", "SERVICE_OVERVIEW", "SOLUTIONS_OVERVIEW", "PRICING_PLANS"].includes(role)
            ? "MULTIPLE_OFFERINGS"
            : "SINGLE_OFFERING";
    ...
    return { drafts: applyRepeatedRepresentativeness(drafts), reasonCodes: drafts.length ? [] : ["NO_QUALIFYING_OFFERING_CONTEXT"] };
  }
}
...
function deterministicLanguageCode(text: string): string | null {
  ...
  const latin = tokens.filter((token) => /^[a-z0-9'%-]+$/i.test(token)).length / tokens.length;
  const common = tokens.filter((token) => ENGLISH_COMMON.has(token)).length;
  return latin >= 0.9 && common >= Math.max(3, Math.floor(tokens.length * 0.02)) ? "en" : null;
}
...
export class CommunicationLanguageSignalsNormalizer implements DataExtractionEvidenceNormalizer {
  readonly capabilityId = "observed_brand_communication_language_signals" as const;

  normalize(input: DataExtractionNormalizationInput): DataExtractionNormalizationResult {
    const drafts: NormalizedEvidenceDraft[] = [];
    for (const source of input.sources) {
      const messageRefs = parentMessagingRefsForSource(source, input.parentEvidence);
      if (messageRefs.length === 0) continue;
      const htmlLang = source.acquiredSourceBody?.match(/<html[^>]+lang=["']([a-zA-Z]{2})(?:-[a-zA-Z]{2})?["']/i)?.[1]?.toLowerCase();
      const languageCode = htmlLang ?? deterministicLanguageCode(source.normalizedText);
    ...
    return { drafts: applyRepeatedRepresentativeness(drafts), reasonCodes: drafts.length ? [] : ["NO_CONTRACT_VALID_LANGUAGE_SIGNAL"] };
  }
}
...
function constraintMatch(text: string): ConstraintMatch | null {
  ...
  if (/\b(never|must not|do not|don't|prohibited|avoid)\b/.test(value)) {
    return { type: "EXPLICIT_BRAND_AUTHORED_PROHIBITION", polarity: "RESTRICTION" };
  }
  if (/\b(must|always|required to|use the term|refer to .* as|call .* a)\b/.test(value)) {
    return { type: "EXPLICIT_REQUIRED_TERMINOLOGY", polarity: "AFFIRMATIVE" };
  }
  if (/\b(disclaimer|claim|compliance|must include|must display)\b/.test(value) && /\b(must|required|do not|never|always)\b/.test(value)) {
    return { type: "EXPLICIT_CLAIM_OR_DISCLAIMER_RULE", polarity: "RESTRICTION" };
  }
  if (/\b(please use|we use only|we always say|our standard is)\b/.test(value)) {
    return { type: "PERSISTENT_OWNED_SOURCE_INSTRUCTION", polarity: "AFFIRMATIVE" };
  }
  return null;
}
...
export class CommunicationConstraintEvidenceNormalizer implements DataExtractionEvidenceNormalizer {
  ...
  normalize(input: DataExtractionNormalizationInput): DataExtractionNormalizationResult {
    const drafts: NormalizedEvidenceDraft[] = [];
    const parentsBySource = new Map<string, DataExtractionEvidenceItemRecord[]>();
    for (const item of input.parentEvidence) {
      if (!["owned_website.brand_messaging", "owned_website.brand_company_context"].includes(item.capabilityId)) continue;
    ...
    for (const source of input.sources) {
      const parents = parentsBySource.get(`${source.resource.resourceRef}|${source.capture.captureRef}`) ?? [];
      ...
      for (const parent of parents) {
        ...
        const payload = {
          ...
          repetition_count: 1,
          source_instruction_scope: source.resource.pageRole === "OFFERING_DETAIL" ? "OFFERING_SPECIFIC" : representativePage(source.resource) ? "BRAND_LEVEL" : "CONTEXT_SPECIFIC",
        } as const;
        drafts.push(
          draft(
            this.capabilityId,
...
export function normalizerFor(
  ...
  const normalizer = WAVE1_NORMALIZERS.find((candidate) => candidate.capabilityId === capabilityId);
...
export function conservativeQuality(
  ...
  if (sources.length === 0) {
    return { state: "UNAVAILABLE", failureCategories: ["INSUFFICIENT_RELEVANT_CONTENT"], detailCodes: ["NO_DURABLE_NORMALIZATION_SOURCE"] };
  }
  const states = sources.map((source) => source.capture.acquisitionQuality.state);
  ...
  return {
    state,
    failureCategories: [...new Set(sources.flatMap((source) => source.capture.acquisitionQuality.failureCategories))],
    detailCodes: [...new Set(sources.flatMap((source) => source.capture.acquisitionQuality.detailCodes))],
  };
}

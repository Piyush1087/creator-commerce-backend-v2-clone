...
export interface ResourceRepository {
  ...
  findByCanonicalIdentity(brandId: BrandId, sourceClass: EvidenceSourceClass, canonicalResourceKey: string): Promise<DataExtractionResourceRecord | null>;
...
export interface CaptureRepository {
  ...
  findByAcquisitionRequestKey(brandId: BrandId, acquisitionRequestKey: string): Promise<DataExtractionCaptureRecord | null>;
  findLatestForResource(brandId: BrandId, resourceRef: ResourceRef): Promise<DataExtractionCaptureRecord | null>;
  listForCapabilityExecution(brandId: BrandId, capabilityExecutionRef: CapabilityExecutionRef): Promise<readonly DataExtractionCaptureRecord[]>;
  markCompleted(brandId: BrandId, captureRef: CaptureRef, result: CompleteCaptureInput): Promise<DataExtractionCaptureRecord>;
  markFailed(brandId: BrandId, captureRef: CaptureRef, result: FailCaptureInput): Promise<DataExtractionCaptureRecord>;
...
export interface ContentArtifactRepository {
  ...
  listForCapture(brandId: BrandId, captureRef: CaptureRef): Promise<readonly DataExtractionContentArtifactRecord[]>;
  insert(record: DataExtractionContentArtifactRecord): Promise<void>;
}

export interface EvidenceItemRepository {
  insertOrGetExact(record: DataExtractionEvidenceItemRecord): Promise<DataExtractionEvidenceItemRecord>;
  ...
  listByCapability(brandId: BrandId, capabilityId: EvidenceCapabilityId): Promise<readonly DataExtractionEvidenceItemRecord[]>;
  listByCapture(brandId: BrandId, captureRef: CaptureRef): Promise<readonly DataExtractionEvidenceItemRecord[]>;
  insert(record: DataExtractionEvidenceItemRecord): Promise<void>;
}

export interface SemanticObservationRepository {
  createOrGet(brandId: BrandId, key: SemanticObservationKey, capabilityId: EvidenceCapabilityId): Promise<DataExtractionSemanticObservationRecord>;
  ...
  attachSupport(brandId: BrandId, key: SemanticObservationKey, evidenceRef: EvidenceRef): Promise<DataExtractionSemanticObservationRecord>;
  listSupport(brandId: BrandId, key: SemanticObservationKey): Promise<readonly EvidenceRef[]>;
  relateEquivalent(brandId: BrandId, sourceKey: SemanticObservationKey, targetKey: SemanticObservationKey): Promise<void>;
  relateConflict(brandId: BrandId, sourceKey: SemanticObservationKey, targetKey: SemanticObservationKey): Promise<void>;
  relate(brandId: BrandId, sourceKey: SemanticObservationKey, targetKey: SemanticObservationKey, relationType: SemanticObservationRelationType): Promise<void>;
  insert(record: DataExtractionSemanticObservationRecord): Promise<void>;
}

export interface CapabilityExecutionRepository {
  createOrGet(input: CreateCapabilityExecutionInput): Promise<DataExtractionCapabilityExecutionRecord>;
  findByRef(brandId: BrandId, ref: CapabilityExecutionRef): Promise<DataExtractionCapabilityExecutionRecord | null>;
  findByRequestKey(brandId: BrandId, requestKey: string): Promise<DataExtractionCapabilityExecutionRecord | null>;
  findLatestReusable(brandId: BrandId, capabilityId: EvidenceCapabilityId): Promise<DataExtractionCapabilityExecutionRecord | null>;
  complete(brandId: BrandId, ref: CapabilityExecutionRef, result: CompleteCapabilityExecutionInput): Promise<DataExtractionCapabilityExecutionRecord>;
  insert(record: DataExtractionCapabilityExecutionRecord): Promise<void>;
}

export interface CapabilityResourceRepository {
  attach(brandId: BrandId, capabilityExecutionRef: CapabilityExecutionRef, resourceRef: ResourceRef): Promise<void>;
  listForExecution(brandId: BrandId, capabilityExecutionRef: CapabilityExecutionRef): Promise<readonly ResourceRef[]>;
}

export interface CapabilityEvidenceRepository {
  attach(brandId: BrandId, capabilityExecutionRef: CapabilityExecutionRef, evidenceRef: EvidenceRef): Promise<void>;
  listEvidenceForExecution(brandId: BrandId, capabilityExecutionRef: CapabilityExecutionRef): Promise<readonly EvidenceRef[]>;
}

export interface FreshnessAssessmentRepository {
  record(input: RecordFreshnessAssessmentInput): Promise<void>;
  listForTarget(brandId: BrandId, targetType: DataExtractionFreshnessAssessment["targetType"], targetRef: string): Promise<readonly DataExtractionFreshnessAssessment[]>;
  latestForTarget(brandId: BrandId, targetType: DataExtractionFreshnessAssessment["targetType"], targetRef: string): Promise<DataExtractionFreshnessAssessment | null>;
}

export interface ProviderExecutionLinkRepository {
  attachToCapture(brandId: BrandId, captureRef: CaptureRef, providerExecutionRef: ProviderExecutionRef, attemptRole: string): Promise<void>;
  attachToCapabilityExecution(brandId: BrandId, capabilityExecutionRef: CapabilityExecutionRef, providerExecutionRef: ProviderExecutionRef, attemptRole: string): Promise<void>;
  listForCapture(brandId: BrandId, captureRef: CaptureRef): Promise<readonly DataExtractionProviderExecutionLink[]>;
  listForCapabilityExecution(brandId: BrandId, capabilityExecutionRef: CapabilityExecutionRef): Promise<readonly DataExtractionProviderExecutionLink[]>;
}

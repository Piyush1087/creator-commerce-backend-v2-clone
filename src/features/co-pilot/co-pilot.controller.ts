...
export class CoPilotController {
  ...
  async createThread(
    ...
    const scope = this.ownerScope(req, brandProfileId);
    const thread = await this.threads.createThread(scope, {
      title: body.title,
    ...
    const messages = await this.threads.listMessages(scope, thread.id);
  ...
  async listThreads(
    ...
    const rows = await this.threads.listThreads(
      this.ownerScope(req, brandProfileId),
      query,
    );
  ...
  async getThread(
    ...
    const scope = this.ownerScope(req, brandProfileId);
    const thread = await this.threads.getThread(scope, threadId);

    if (!thread) {
      throw new NotFoundException("Thread not found");
    }

    const messages = await this.threads.listMessages(scope, threadId);
  ...
  async patchThread(
    ...
    const updated = await this.threads.patchThread(
      this.ownerScope(req, brandProfileId),
      threadId,
      {
        title: body.title,
  ...
  async listMessages(
    ...
    const messages = await this.threads.listMessages(
      this.ownerScope(req, brandProfileId),
      threadId,
    );
  ...
  async confirmHitl(
    ...
    const message =
      result.message ?? result.hitlResolution?.summary ?? "Action confirmed.";
  ...
  private async handleMessage(
    ...
    const existingMessages =
      (await this.threads.listMessages(scope, threadId)) ?? [];

    const scopeContext =
      body.scopeContext ??
      thread.scopeContext ??
      CoPilotScopeContext.BRAND_CENTRE;
  ...
  private ownerScope(
    ...
    return { brandProfileId, userId: req.user.id };
  }

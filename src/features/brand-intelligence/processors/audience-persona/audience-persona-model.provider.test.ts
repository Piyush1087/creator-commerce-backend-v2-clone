...
describe("StructuredAudiencePersonaModelProvider", () => {
  it("uses deterministic bounded production dispatch", async () => {
    const execute = vi.fn(async () => ({
      payload: { ok: true },
      telemetry: { attemptCount: 1 },
    }));
    const provider = new StructuredAudiencePersonaModelProvider(
  ...
  it.each([
    ...
    async (code, retryable) => {
      ...
      await expect(provider.generate(request)).rejects.toEqual(
        expect.objectContaining<Partial<AudiencePersonaProviderError>>({
          code,
          retryable,
        }),
      );

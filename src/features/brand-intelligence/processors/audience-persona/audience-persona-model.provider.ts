...
export const AUDIENCE_PERSONA_MODEL_PROVIDER = Symbol(
  "AUDIENCE_PERSONA_MODEL_PROVIDER",
);

export interface AudiencePersonaModelRequest {
...
export interface AudiencePersonaModelResult {
  readonly output: unknown;
  readonly providerAttemptCount: number;
}

export interface AudiencePersonaModelProvider {
  generate(
    request: AudiencePersonaModelRequest,
  ): Promise<AudiencePersonaModelResult>;
}

export class AudiencePersonaProviderError extends Error {
  constructor(
    readonly code: string,
    readonly retryable: boolean,
  ) {
    super(code);
    this.name = "AudiencePersonaProviderError";
  }
}

@Injectable()
export class StructuredAudiencePersonaModelProvider implements AudiencePersonaModelProvider {
  constructor(
  ...
  async generate(
    ...
    try {
      const result = await this.structuredExecution.execute({
        ...
        capabilityId: "audience_persona_synthesis",
        modelId: this.config.get<string>(
          "AUDIENCE_PERSONA_MODEL_ID",
          "gemini-2.5-flash",
        ),
        ...
        timeoutMs: this.config.get<number>(
          "AUDIENCE_PERSONA_MODEL_TIMEOUT_MS",
          60_000,
        ),
        // W1.0D is the processor retry owner. Provider-internal retry is one call.
        maxAttempts: 1,
        schemaName: "audience_persona_synthesis_1_0",
        temperature: 0,
      });
      return {
      ...
      if (!(error instanceof StructuredEvidenceExecutionError)) {
        throw new AudiencePersonaProviderError("PROVIDER_ERROR", true);
      }
      ...
      throw new AudiencePersonaProviderError(error.code, retryable);
    }
  }
}

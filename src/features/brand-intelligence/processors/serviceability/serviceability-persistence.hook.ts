...
import {
  ...
  lockServiceabilityCanonicalBasis,
...
import {
  ...
  type ServiceabilityPersistencePayload,
...
/** W1.0D owns the transaction/live lease. Only frozen serviceability paths are mapped here. */
@Injectable()
export class ServiceabilityPersistenceHook implements ProcessorSuccessPersistenceHook {
  constructor(
    ...
    private readonly catalogue: ServiceabilityStateRepository,
  ) {}
  ...
  async persistBeforeCompletion(
    ...
    if (
      claim.processorExecution.processorId !== "serviceability_synthesis" ||
      !raw ||
      typeof raw !== "object" ||
      (raw as { kind?: unknown }).kind !== "SERVICEABILITY_V1"
    )
      serviceabilityInvalid("SERVICEABILITY_INVALID_PERSISTENCE_PAYLOAD");
    const {
      output,
      prepared,
      current: before,
    } = raw as ServiceabilityPersistencePayload;
    ...
    await lockServiceabilityCanonicalBasis(tx, prepared.canonicalState);
    const planned = serviceabilityComponentPlan(
      output,
      before,
    ...
    if (serviceabilityFingerprint(live) !== serviceabilityFingerprint(before))
      throw new ProcessorExecutorFailure({
        category: "RETRYABLE_TECHNICAL",
        code: "SERVICEABILITY_CURRENT_BASIS_CHANGED",
      });
    ...
    for (const plan of plans) {
      ...
      const meta = plan.metadata
        ...
        : {};
      const authority = plan.metadata?.authority ?? "CREATOR_SHOP_DERIVED";
      ...
      components.push({
        ...
        componentContractId: SERVICEABILITY_OBJECT,
      ...
      for (const ref of plan.metadata?.evidence_refs ?? []) {
        ...
        if (!item || item.brandId !== execution.brandId)
          serviceabilityInvalid("SERVICEABILITY_UNKNOWN_EVIDENCE_REF");
      ...
      for (const entry of serviceabilityBusinessEntries(
        prepared.canonicalState,
      )) {
        const encodedRef = serviceabilityBusinessRef(
          entry.semantic,
          entry.businessStateReference,
        );
        if (!plan.metadata?.business_state_refs?.includes(encodedRef)) continue;
    ...
    if (!validation.valid)
      serviceabilityInvalid(
        `SERVICEABILITY_PERSISTENCE_${validation.issues[0]?.code}`,
      );
    await this.generations.persistInTransaction(tx, {
      object: {
        id: uuid(`${execution.id}:${SERVICEABILITY_OBJECT}`),
        brandId: execution.brandId,
        objectSemanticId: SERVICEABILITY_OBJECT,
        objectContractId: "serviceability_objects",
        ...
        valueState:
          output.serviceability_profile === null ? "EXPLICIT_NULL" : "VALUE",
        valuePayload:
          output.serviceability_profile === null
            ? Prisma.JsonNull
            : json(output.serviceability_profile),
        valueHash: sha256CanonicalExecution(output.serviceability_profile),
    ...
    const applied = await this.transitions.transitionInTransaction(tx, {
      action: {
        id: uuid(`${execution.id}:serviceability-transition`),
        brandId: execution.brandId,
        ...
        reasonCode: "VALIDATED_SERVICEABILITY_RECONCILIATION",
        processorExecutionId: execution.id,
      },
      decisions: applicable.map((p) => ({
        ...
        discrepancyCode: "PROTECTED_SERVICEABILITY_CONFLICT",
      })),
    });
    if (
      ...
      serviceabilityInvalid("SERVICEABILITY_TRANSITION_REJECTED");
  }
}

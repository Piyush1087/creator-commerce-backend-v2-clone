...
import { differentiationOutputReadiness } from "./brand-differentiation-plan";
...
import {
  ...
  BrandDifferentiationProviderError,
  type BrandDifferentiationModelProvider,
} from "./brand-differentiation-model.provider";
import {
  BRAND_DIFFERENTIATION_PROMPT_VERSION,
  BRAND_DIFFERENTIATION_SYSTEM_INSTRUCTION,
} from "./brand-differentiation-prompt";
...
function businessManifest(prepared: PreparedProcessorDependencies) {
  return [
    ...
    businessStateRef: differentiationBusinessRef(
      entry.semantic,
...
@Injectable()
export class BrandDifferentiationProcessorExecutor implements ProcessorExecutor {
  readonly processorId = "brand_differentiation";
  constructor(
    private readonly dependencies: ProcessorDependencyPreparationService,
    private readonly contracts: ContractRuntimeRegistry,
    private readonly state: BrandDifferentiationStateRepository,
    private readonly structural: StructuralValidator,
    private readonly semantic: SemanticValidator,
    @Inject(BRAND_DIFFERENTIATION_MODEL_PROVIDER)
    private readonly model: BrandDifferentiationModelProvider,
  ) {}
  ...
  async execute(context: ProcessorExecutorContext) {
    try {
      ...
      const prepared = await this.dependencies.prepare({
        ...
        registryKey,
        activeScope: differentiationScope(
      ...
      if (
        ...
        (prepared.canonicalState.offeringFacts ?? []).some(
          (f) => f.brandId !== execution.brandId,
        )
      )
        throw new ProcessorExecutorFailure({
          category: "VALIDATION_FAILURE",
          code: "DIFFERENTIATION_CROSS_BRAND_INPUT",
        });
      ...
      const generated = await this.model.generate({
        ...
        instruction: BRAND_DIFFERENTIATION_SYSTEM_INSTRUCTION,
        ...
        approvedContext: {
          ...
          canonicalOfferings: (prepared.canonicalState.offeringFacts ?? []).map(
            (fact) => ({
              ...fact,
              businessStateRef: business.find(
                (b) => b.semanticId === `offering:${fact.offeringId}`,
              )!.businessStateRef,
              referenceOnly: true,
            }),
          ),
      ...
      const output = semantic.value as DifferentiationOutput;
      validateDifferentiationIdentity(output, current);
      return {
        readiness: differentiationOutputReadiness(output),
        telemetry: {
          promptVersion: BRAND_DIFFERENTIATION_PROMPT_VERSION,
          providerAttemptCount: generated.providerAttemptCount,
        },
        persistencePayload: {
          kind: "BRAND_DIFFERENTIATION_V1",
          current,
          output,
          prepared,
        } satisfies DifferentiationPersistencePayload,
      };
    } catch (error) {
      if (error instanceof ProcessorExecutorFailure) throw error;
      if (error instanceof BrandDifferentiationProviderError)
        throw new ProcessorExecutorFailure({
          category:
      ...
      throw new ProcessorExecutorFailure({
        ...
        code: "BRAND_DIFFERENTIATION_EXECUTOR_CONFIGURATION_INVALID",
      });
    }
  }
}

...
describe("StructuredBrandCommunicationModelProvider", () => {
  it("uses deterministic bounded production dispatch", async () => {
    const execute = vi.fn(async () => ({
      payload: { ok: true },
      telemetry: { attemptCount: 1 },
    }));
    const provider = new StructuredBrandCommunicationModelProvider(
  ...
  it.each([
    ...
    async (code, retryable) => {
      ...
      await expect(provider.generate(request)).rejects.toEqual(
        expect.objectContaining<Partial<BrandCommunicationProviderError>>({
          code,
          retryable,
        }),
      );

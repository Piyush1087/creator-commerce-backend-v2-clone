...
describe("StructuredBrandMeaningModelProvider", () => {
  it("uses deterministic bounded production dispatch", async () => {
    const execute = vi.fn(async () => ({
      payload: { ok: true },
      telemetry: { attemptCount: 1 },
    }));
    const provider = new StructuredBrandMeaningModelProvider(
  ...
  it.each([
    ...
    async (code, retryable) => {
      ...
      await expect(provider.generate(request)).rejects.toEqual(
        expect.objectContaining<Partial<BrandMeaningProviderError>>({
          code,
          retryable,
        }),
      );

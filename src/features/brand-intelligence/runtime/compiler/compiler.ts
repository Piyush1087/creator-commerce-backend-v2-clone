...
export async function executeProfile(
  ...
  const pending = new Map(profile.tasks.map((t) => [t.id, t]));
  ...
  while (pending.size) {
    for (const [id, task] of [...pending]) {
      const failed = (task.dependsOn ?? []).find((dep) => {
        const s = results.get(dep)?.state;
        return s && s !== "SUCCEEDED";
      });
      if (failed && results.has(failed)) {
        const depState = results.get(failed)?.state;
        results.set(id, {
          taskId: id,
          state:
            depState === "STOPPED_EXPECTED"
              ? "STOPPED_EXPECTED"
              : "SKIPPED_DEPENDENCY",
          error: {
            code:
    ...
    const ready = [...pending.values()].filter((task) =>
      (task.dependsOn ?? []).every(
        (dep) => results.get(dep)?.state === "SUCCEEDED",
      ),
    );
    if (!ready.length) {
    ...
    const batch = await Promise.all(
      ready.map(async (task) => {
        try {
          const args: CompilerTaskContext = {
            executionId,
            task,
            request,
  ...
  const ordered = profile.tasks
    ...
    .filter(Boolean);
  const stoppedExpected = ordered.some((r) => r.state === "STOPPED_EXPECTED");
  const requiredFailures = profile.tasks.some(
    (t) =>
      t.required !== false &&
      results.get(t.id)?.state !== "SUCCEEDED" &&
      results.get(t.id)?.state !== "STOPPED_EXPECTED",
  );
  const anySuccess = ordered.some((r) => r.state === "SUCCEEDED");
  const allSuccess =
    ordered.length === profile.tasks.length &&
    ordered.every((r) => r.state === "SUCCEEDED");

  let state: ExecutionState;
  if (stoppedExpected && !allSuccess) state = "STOPPED_EXPECTED";
  else if (allSuccess) state = "SUCCEEDED";
  else if (requiredFailures) state = "FAILED";
  else if (anySuccess) state = "PARTIAL";
  else state = "FAILED";

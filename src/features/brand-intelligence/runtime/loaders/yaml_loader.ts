...
export class SafeYamlLoader {
  ...
  private resolve(relativePath: string): string {
    ...
    if (path.isAbsolute(normalized) || normalized.includes("..")) {
      throw new RuntimeConfigError(
        "CONFIG_PATH_INVALID",
        `Unsafe config path: ${relativePath}`,
      );
    }
    const root = path.resolve(this.metadataRoot);
    const resolved = path.resolve(root, normalized);
    if (resolved !== root && !resolved.startsWith(root + path.sep)) {
      throw new RuntimeConfigError(
        "CONFIG_PATH_INVALID",
        "Config path escapes metadata root",
      );
    }
    return resolved;
  }
  ...
  async load<T = unknown>(relativePath: string): Promise<T> {
    try {
      ...
      throw new RuntimeConfigError(
        "CONFIG_LOAD_FAILED",
        `Failed to load ${relativePath}: ${error instanceof Error ? error.message : "unknown error"}`,
      );
    }
  }
}

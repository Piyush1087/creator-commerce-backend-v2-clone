...
export const LanguageCodeSchema = z
  .string()
  .regex(/^[a-z]{2}$/, "Expected ISO 639-1 lowercase language code");
export const CurrencyCodeSchema = z
  .string()
  .regex(/^[A-Z]{3}$/, "Expected ISO 4217 uppercase currency code");
...
export function buildIdentityCoreSchema(activeOutputs: IdentityCoreOutputId[]) {
  if (activeOutputs.length === 0) {
    throw new Error("At least one identity_core output is required");
  }
...
export const IndustryNicheSchema = z
  .object({ industry_niche: z.string().nullable() })
  .strict();
  ...
  .superRefine((value, ctx) => {
      ...
      for (const key of [
        ...
        if (value[key] !== null) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: [key],
            message: `GLOBAL requires ${key}=null`,
          });
        }
      }
    }
    if (value.scope === "COUNTRY") {
      if (!value.country_code) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["country_code"],
          message: "COUNTRY requires country_code",
        });
      }
      if (value.locality !== null) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["locality"],
          message: "COUNTRY requires locality=null",
        });
      }
    }
    if (value.scope === "LOCALITY" && !value.locality) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["locality"],
        message: "LOCALITY requires locality",
      });
    }
    if (value.scope === "REGION" && !value.region) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["region"],
        message: "REGION requires region",
      });
    }
  });
  ...
  .superRefine((value, ctx) => {
    if (!value.markets_served.some((market) => market.is_primary)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["markets_served"],
        message: "At least one market must be primary",
      });
    }
  });
...
export function validateGatekeeperTaxonomy(
  ...
  if (legacyIndustryValues.includes(output.industry)) {

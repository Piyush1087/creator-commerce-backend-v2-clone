...
export function validateProcessorOutput(
  ...
  if (request.processor_id === "identity_core") {
    try {
      const schema = buildIdentityCoreSchema(
        request.active_outputs as IdentityCoreOutputId[],
      );
      ...
      return fail("CONFIGURATION", [
        {
          path: [],
          code: "INVALID_ACTIVE_OUTPUT_CONFIGURATION",
          message:
            error instanceof Error
              ? error.message
              : "Invalid identity_core active output configuration",
        },
      ]);
    }
  }
  ...
  if (
    request.processor_id === "industry_classification" &&
    request.processor_scope === "gatekeeper"
  ) {
    const structural = GatekeeperSchema.safeParse(request.raw_output);
    if (!structural.success) {
      return fail("STRUCTURAL", issuesFromZod(structural.error));
    }
    if (!request.taxonomy) {
      return fail("CONFIGURATION", [
        {
          path: [],
          code: "CANONICAL_TAXONOMY_REQUIRED",
          message:
            "Gatekeeper semantic validation requires canonical Industry/Sub-industry taxonomy",
        },
      ]);
    }
  ...
  if (
    request.processor_id === "industry_classification" &&
    request.processor_scope === "industry_niche"
  ) {
  ...
  return fail("CONFIGURATION", [
    {
      ...
      message: `No validator configured for ${request.processor_id}${request.processor_scope ? `.${request.processor_scope}` : ""}`,
    },
  ]);
}

...
describe.skipIf(process.env.C06_PAYOUTS_DATABASE_TEST !== "true")(
  ...
  () => {
    ...
    const measuredQueries: string[] = [];
    let captureMeasuredQueries = false;
    db.$on("query", ({ query }) => {
      if (captureMeasuredQueries) measuredQueries.push(query);
    });
    ...
    it("proves actor isolation and exact canonical read truth without provider actions", async () => {
      ...
      captureMeasuredQueries = false;
      expect(
        measuredQueries.filter((query) =>

...
const CURSOR_KEY_DOMAIN = "creator-shop:creator-payouts:v1:cursor";
...
export type CreatorPayoutsCursorBoundary = {
  readonly asOf: Date;
...
function invalidCursor(): BadRequestException {
  return new BadRequestException({
    code: "CREATOR_PAYOUTS_CURSOR_INVALID",
    message: "The Creator Payouts continuation cursor is invalid",
  });
}
...
function isPayload(value: unknown): value is CursorPayload {
  ...
  return (
    ...
    typeof row.subjectCreatorProfileId === "string" &&
...
@Injectable()
export class CreatorPayoutsCursorCodec {
  ...
  encode(input: {
    ...
    const payload: CursorPayload = {
      ...
      subjectCreatorProfileId: scope.subjectCreatorProfileId,

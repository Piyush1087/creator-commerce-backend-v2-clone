...
const payoutTermsSchema = z.enum([
  "NET_7",
  "NET_15",
  "NET_30",
  "NET_45",
...
const strategySchema = z
  .object({
    ...
    objective: canonicalCampaignObjectiveSchema,
    ...
    if (value.publishing_schedule === "SCHEDULED") {
      if (!value.publish_from) {
        ctx.addIssue({
          ...
          path: ["publish_from"],
          message: "Scheduled campaigns require publish_from.",
        });
      }
      if (!value.publish_until) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["publish_until"],
          message: "Scheduled campaigns require publish_until.",
        });
      }
      if (
        value.publish_from &&
        utcDayStart(new Date(value.publish_from)) < utcDayStart(new Date())
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["publish_from"],
          message: "publish_from cannot be in the past.",
        });
      }
      if (
        value.publish_from &&
        value.publish_until &&
        new Date(value.publish_until) < new Date(value.publish_from)
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["publish_until"],
          message: "publish_until must be on or after publish_from.",
        });
      }
    }
    if (
      value.publishing_schedule === "EVERGREEN" &&
      (value.publish_from != null || value.publish_until != null)
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["publish_until"],
        message: "Evergreen campaigns cannot supply publishing dates.",
      });
    }
  });
...
const targetingSchema = z
  ...
  .superRefine((value, ctx) => {
    if (
      value.maximum_followers != null &&
      value.maximum_followers <= value.minimum_followers
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["maximum_followers"],
        message: "maximum_followers must be greater than minimum_followers.",
      });
    }
    if (value.audience_age_max < value.audience_age_min) {
      ctx.addIssue({
        ...
        message: "audience_age_max must be >= audience_age_min.",
      });
    }
  });
...
const commercialsSchema = z
  ...
  .superRefine((value, ctx) => {
    if (
      !value.receives_brand_support &&
      (value.brand_support_type != null ||
        value.brand_support_estimated_value != null)
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["receives_brand_support"],
        message: "Brand-support fields must be empty when support is disabled.",
      });
    }
    if (value.receives_brand_support && !value.brand_support_type) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["brand_support_type"],
        message: "brand_support_type is required when support is enabled.",
      });
    }
    if (value.total_campaign_budget < value.commercial_offer) {
      ctx.addIssue({
        ...
        message: "total_campaign_budget must be >= commercial_offer.",
      });
    }
  });
...
export type CanonicalCampaignWizardPayload = z.infer<
  typeof canonicalCampaignWizardSchema
>;

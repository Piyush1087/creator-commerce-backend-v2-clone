...
export class NotificationDispatchService {
  ...
  async dispatch(input: NotificationDispatchInput): Promise<{ job_id: string }> {
    const definition = getEventDefinition(input.eventType);
    if (!definition) {
      throw new BadRequestException(`Unknown notification event: ${input.eventType}`);
    }
    ...
    const job = await this.prisma.notificationJob.create({
      data: {
        ...
        urgencyLevel: input.urgencyLevel ?? definition.urgencyLevel,
